...
with open('customer_signin.txt','r') as read_user:
        user = str(read_user.readlines())
        print('Sign in Successful ',user)
        user = user.replace('[','')
        user = user.replace(']','')
        user = user.replace("'",'')
        with open('customer_signin.txt','w') as delete_user:
...
def Signin_success(username):
        ...
        login_success_text = 'Welcome %s'%(username)
        speak(login_success_text)
